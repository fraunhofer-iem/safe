 ...
 *
 * Persisted fields are kept deliberately lean: a participant id supplied by the
 * experimenter, the telemetry directory (defaults to `<project>/.idea/safe-telemetry/`)
 * and the on/off flag. The session id is **not** persisted — it rolls every IDE
 * start, so a participant who restarts mid-study produces two distinguishable
...
class StudyModeSettings : PersistentStateComponent<StudyModeSettings.State> {
    ...
    /**
     * Lazily generated per-IDE-start session id. Stays constant for the lifetime
     * of the JVM and rolls on the next IDE launch — gives analysis a clear
     * boundary without requiring participants to type anything.
     */
