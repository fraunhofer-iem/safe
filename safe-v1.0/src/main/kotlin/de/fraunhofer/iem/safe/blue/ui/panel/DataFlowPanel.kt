...
import de.fraunhofer.iem.safe.blue.ui.icons.IconUtils
import de.fraunhofer.iem.safe.blue.sast.DataFlowCategory
import de.fraunhofer.iem.safe.blue.sast.DataFlowElement
import de.fraunhofer.iem.safe.blue.sast.Issue
import de.fraunhofer.iem.safe.blue.ui.icons.PluginIcons
import de.fraunhofer.iem.safe.blue.util.MethodUtil
import java.awt.*
import java.io.File
import javax.swing.*
...
class DataFlowPanel(val project: Project) : JPanel() {
    ...
    // Built once (in setupNavigationBar) and reused across clicks. Recreating
    // the buttons inside the action listener every time used to drop the
    // clicked-button mid-event, which made the panel look like it was eating
    // clicks until the user pressed several times.
    private val prevButton = JButton(AllIcons.General.ChevronLeft).apply {
    ...
    private val nextButton = JButton(AllIcons.General.ChevronRight).apply {
        toolTipText = "Next"
        addActionListener {
    ...
    fun showFileContent(issue: Issue) {
        ...
        if (issue.hasDataFlowTrace) {
            // Land on the first step automatically — previously the panel sat at
            // index -1, leaving the description label blank until the user pressed
            // Next once. Starting at 0 highlights the source straight away and
            // gives the user something to read on the very first render.
            currentTraceIndex = 0
            annotateDataFlow(issue, editor)
            setupNavigationBar()
            add(navigationPanel, BorderLayout.NORTH)
            add(dataFlowPanel, BorderLayout.SOUTH)
            // Position the caret + label on the first element so the description
            // is populated when the panel becomes visible.
            scrollToCurrentTrace()
        }
     ...
     *
     * Skips trace elements whose offsets fall outside the loaded document — that
     * happens when the SARIF / JSON file was produced against a different version
     * of the source than the one currently on disk (or when the trace points at a
     * file other than the issue's primary file). Without this guard, the platform
     * throws `IllegalArgumentException("Invalid offsets: start=…; end=…; document length=…")`.
     ***/
    private fun annotateDataFlow(issue: Issue, editor: Editor?) {
        ...
        issue.dataFlowTrace?.forEach { dataFlowElement ->
            if (!isValidRange(dataFlowElement.startOffset, dataFlowElement.endOffset, docLength)) {
                return@forEach
            }
    ...
    /***
     * Populates [navigationPanel] with the prev / next buttons and the data-flow
     * description label. Called once per finding (from [showFileContent]) — the
     * buttons themselves live for the lifetime of this panel and only have their
     * enabled state refreshed on navigation. Rebuilding them inside the action
     * listener used to delete the very button the user just clicked, causing
     * clicks to feel like they were getting swallowed.
     */
    ...
    /** Updates the prev / next button's enabled state based on [currentTraceIndex]. */
    private fun refreshNavigationButtons() {
        val size = dataFlowTrace?.size ?: 0
        prevButton.isEnabled = currentTraceIndex > 0
        nextButton.isEnabled = currentTraceIndex < size - 1
    }
    ...
    private fun scrollToCurrentTrace() {
        ...
        val docLength = editor.document.textLength
        if (!isValidRange(currentElement.startOffset, currentElement.endOffset, docLength)) return
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            ...
            for (expr in expressions) {
                ...
                if (SrmFinder.isSRM(signature)) {
                    ...
                    highlightTasks.add {
                        val targetEditor = editor ?: return@add
                        if (!isValidRange(start, end, targetEditor.document.textLength)) return@add
                        targetEditor.markupModel.addRangeHighlighter(
                            start, end,
                            HighlighterLayer.ERROR,
                            TextAttributes(
    ...
    /**
     * Returns true when `[start, end)` is a valid range inside a document of
     * `docLength` characters. SARIF / JSON traces sometimes carry offsets that
     * pre-date file edits or refer to a different file altogether — feeding
     * those to [com.intellij.openapi.editor.markup.MarkupModel.addRangeHighlighter]
     * crashes with `Invalid offsets`. Skipping is preferable to silently
     * truncating, since a clamped range would highlight the wrong code.
     */
    private fun isValidRange(start: Int, end: Int, docLength: Int): Boolean =
        start in 0..end && end <= docLength
}